...
int check=0;
const int Set=3;
String Tag[Set] = {"33B03427","ID2","ID3"};

#define g_led 8 // Green LED Pin
#define y_led 7 // Yellow LED Pin
...
void loop() {//Wait until new tag is available
  while(getID()){    
    ...
    if(tagID == Tag[i]){check=1;
      ...
      for(int angle=140; angle>=30; angle--){servoPulse(angle);}
      delay(5000);
      digitalWrite(g_led, 0);
      digitalWrite(y_led, 1);
      digitalWrite(r_led, 0);
      delay(1000);
      for(int angle=30; angle<=140; angle++){servoPulse(angle);}
      i=Set;
     }
    }
